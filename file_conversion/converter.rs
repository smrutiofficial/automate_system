...
fn main() {
    ...
    let input_paths: Vec<&str> = args.iter().skip(1).take(args.len() - 3).map(|s| s.as_str()).collect();
    let output_path = &args[args.len() - 2];
    let conversion_type = &args[args.len() - 1];

    convert_files(&input_paths, output_path, conversion_type);
}

fn convert_files(input_paths: &[&str], output_path: &str, conversion_type: &str) {
    for input_path in input_paths {
        convert_file(input_path, output_path, conversion_type);
    }
}
